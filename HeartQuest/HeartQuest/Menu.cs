...
namespace HeartQuest
{
    class Menu
    {
        ...
        private void DrawText(SpriteBatch spriteBatch)
        {
            ...
            {
                Color color = (j == Selection) ? Color.Red: Color.Black;
                Vector2 textSize = Font.MeasureString(Options[j]);
                spriteBatch.DrawString(Font, Options[j], Position + (new Vector2(0.5f * ((Width-2) * CenterPiece.Width - textSize.X), StraightPiece.Height + (j*textSize.Y))), color);
            }
        }
